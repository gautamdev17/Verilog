...
// use dynamic part-select base +: width and compute start before the loop
`timescale 1ns/1ps
...
module switch_allocator(
  ...
  input  logic clk,
  input  logic rst,
  output reg [14:0]   selsignal,
  output reg [4:0]    fifo_pop
);
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      for (i=0;i<5;i=i+1) begin
        k = 0;
        // initialize j from rrbptr using MSB-first dynamic slice (-: 3)
        for (j = rrbptr[((i+1)*3-1)-:3];k != 5;j = (j==4)?0:j+1) begin : IN_LOOP
          k = k + 1;
          if (reqMat[i*5+j] == 1'b1) begin
            ...
            selsignal[((i+1)*3-1)-:3] <= j[2:0];
            rrbptr[((i+1)*3-1)-:3] <= (j==4)?0:j+1;
            fifo_pop[i] <= 1'b1;
            disable IN_LOOP;
          end
        end
